...
extern const struct dai_ops esai_ops;

static inline void esai_write(struct dai *dai, uint32_t reg, uint32_t value)
{
	io_reg_write(dai_base(dai) + reg, value);
}

static inline uint32_t esai_read(struct dai *dai, uint32_t reg)
{
	return io_reg_read(dai_base(dai) + reg);
}

static inline void esai_update_bits(struct dai *dai, uint32_t reg, uint32_t mask,
	uint32_t value)
{
